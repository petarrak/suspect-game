...
import { joinLiarRoom } from "@/lib/liar";

export default function JoinLiarPage() {
  ...
  useEffect(() => {
    ...
    const cleaned = qrCode.trim().toUpperCase().slice(0, 6);
    setCode(cleaned);

    if (cleaned.length === 6) {
      window.setTimeout(() => nicknameRef.current?.focus(), 100);
    }
  }, []);
  ...
  async function handleJoin() {
    ...
    try {
      const joinedCode = await joinLiarRoom(cleanedCode, trimmed, avatar);
      router.push(`/liar/room/${joinedCode}`);
    } catch (e: any) {
