...
const CATEGORIES: {
  ...
  { value: "COUNTRIES", emoji: "🌍", hr: "Države", en: "Countries" },
...
export default function LiarRoomPage() {
  ...
  const { room, players, loading, error } = useLiarRoomRealtime(code);

  const meId = getRememberedLiarPlayerId(code);
  const me = players.find((player) => player.id === meId) ?? null;
  ...
  async function changeSettings(
    patch: Parameters<typeof updateLiarSettings>[1]
  ) {
    if (!room || !me?.is_host || settingsLoading) return;

    setSettingsLoading(true);
    setSettingsError(null);

    try {
      await updateLiarSettings(room.id, patch);
    } catch (e: any) {
      setSettingsError(e?.message ?? "Could not update settings.");
    } finally {
  ...
  async function handleKick(playerId: string) {
    if (!room || !me?.is_host || kickingId) return;

    setKickingId(playerId);
    setSettingsError(null);

    try {
      await kickLiarPlayer(room.id, playerId);
    } catch (e: any) {
      setSettingsError(e?.message ?? "Could not kick player.");
    } finally {
      setKickingId(null);
    }
  }
    ...
    return (
      <main className="min-h-screen flex items-center justify-center">
        <p className="text-white/50">
          {language === "hr" ? "Učitavanje Liar sobe..." : "Loading Liar room..."}
        </p>
      </main>
    );
  }
  ...
  if (error || !room) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-5 p-6">
        <p className="text-accent">{error ?? "Room not found."}</p>
        <Button variant="secondary" onClick={() => router.push("/liar")}>
          {t("back")}
        </Button>
      </main>
    );
    ...
    return (
      <main className="min-h-screen max-w-md mx-auto flex flex-col items-center justify-center gap-5 p-6 text-center">
        ...
        <h1 className="text-2xl font-black">
          {language === "hr" ? "Nisi u ovoj sobi" : "You're not in this room"}
        </h1>
        <Button onClick={() => router.push(`/liar/join?code=${room.code}`)}>
  ...
  return (
    <main className="min-h-screen max-w-md mx-auto flex flex-col gap-6 p-6">
      <div className="text-center pt-3">
        ...
        <h1 className="mt-1 text-3xl font-black">LOBBY</h1>
      </div>
      ...
      <section className="flex flex-col gap-3">
        <p className="text-xs font-black uppercase tracking-widest text-white/35">
          {language === "hr" ? "IGRAČI" : "PLAYERS"} ({players.length}/12)
        </p>
        ...
        <section className="card p-5 flex flex-col gap-6">
          <div>
            ...
            <h2 className="mt-1 text-xl font-black">
              {language === "hr" ? "Postavke Liara" : "Liar Settings"}
            </h2>
          </div>
          ...
          <div>
            <div className="flex justify-between mb-3">
              <span className="text-sm font-bold">
                {language === "hr" ? "RUNDE" : "ROUNDS"}
              </span>
              <span className="text-accent font-black">{room.total_rounds}</span>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {ROUND_OPTIONS.map((rounds) => (
                <button
                  ...
                  onClick={() => changeSettings({ total_rounds: rounds })}
          ...
          <div>
            <div className="flex justify-between mb-3">
              <span className="text-sm font-bold">
                {language === "hr" ? "VRIJEME RASPRAVE" : "DISCUSSION TIME"}
              </span>
            ...
            <div className="grid grid-cols-4 gap-2">
              {TIME_OPTIONS.map((seconds) => (
                <button
                  ...
                  disabled={settingsLoading}
                  onClick={() =>
                    changeSettings({ discussion_time: seconds })
                  }
                  className={`rounded-xl border py-3 text-sm font-black ${
                    room.discussion_time === seconds
                      ? "border-accent bg-accent/20"
                      : "border-white/10 bg-black/20 text-white/50"
                  }`}
                >
            ...
            <div className="grid grid-cols-2 gap-2">
              {CATEGORIES.map((category) => (
                <button
                  key={category.value}
                  disabled={settingsLoading}
                  onClick={() => changeSettings({ category: category.value })}
          ...
          {settingsError && (
            <p className="text-sm text-accent">{settingsError}</p>
          )}
        </section>
      )}
      ...
      <div className="mt-auto">
        {me.is_host ? (
          ...
          <p className="text-center text-white/40">
            {language === "hr" ? "Čekamo hosta..." : "Waiting for host..."}
          </p>
        )}
      </div>
    </main>
